const Category = require('../../../models').Category;
const { validationResult } = require('express-validator');
const { _response } = require('../../../utils/functions/response');
const { getPagination, getPagingData } = require('../../../utils/functions/functions');

exports.post = async (req, res) => {
    const { categoryName } = req.body;
    try {
        /* NO CONTENT */
        if (Object.keys(req.body).length === 0) { return res.status(400).json({ message: _response.empty }) }

        /* CHECK VALIDATION */
        let errors = validationResult(req)
        const formatter = (error) => error.msg
        if (!errors.isEmpty()) { return res.status(400).send({ message: errors.formatWith(formatter).mapped() }) }

        /* EXISTING */
        const existingName = await Category.findAll({ where: { categoryName: categoryName } });

        /* CREATE */
        if (existingName.length === 0) {
            const category = await Category.create({ categoryName });
            if (!category) { return res.status(200).json({ message: _response.failed }); }
            return res.status(200).json({ category });
        } else { return res.status(302).json({ message: _response.duplicateValue }) }

    } catch (error) { console.log(error) }
}

exports.getSubCategory = async (req, res) => {

    const { page, size } = req.query;
    if (page === undefined) { page = 1 }
    const { limit, offset } = getPagination(page, size);

    try {
        /* FETCH LIST */
        const data = await Category.findAndCountAll({
            limit: limit,
            offset: offset,
            // attributes: ['id', 'title', 'status'],
            include: { model: SubCategory, required: true }
        });

        const list = getPagingData(data, page, limit);
        if (list.length !== 0) {
            return res.status(200).json({ list });
        } else { return res.status(404).json({ message: _response.empty }); }

    } catch (error) { console.log(error) }

}

exports.update = async (req, res) => {
    const { categoryName } = req.body;
    const id = req.params.id;
    const condition = { id: id };
    const query = { where: { title: title } }

    try {
        /* NO_CONTENT */
        if (Object.keys(req.body).length === 0) { return res.status(400).json({ message: _response.empty }) }

        /* CHECK_VALIDATION */
        let errors = validationResult(req)
        const formatter = (error) => error.msg
        if (!errors.isEmpty()) { return res.status(400).send({ message: errors.formatWith(formatter).mapped() }) }


        /* EXISTING */
        const existingCategory = await Category.findAll(query);

        /* UPDATE */
        if (existingCategory.length === 0) {
            const category = await Category.update({ title }, { where: condition });
            if (!category) { return res.status(200).json({ message: _response.failed }) };
            return res.status(200).json({ message: _response.update });
        } else { return res.status(302).json({ message: _response.duplicateValue }) }

    } catch (error) { console.log(error) }
}

exports.remove = async (req, res) => {
    const id = req.params.id;
    const condition = { id: id };

    try {
        /* DELETE */
        const category = await Category.destroy({ where: condition });

        if (category === 1) {
            return res.status(200).json({ message: _response.delete })
        } else if (category === 0) {
            return res.status(404).json({ message: _response.empty })
        } else { return res.status(500).json({ message: _response.error }) }

    } catch (error) { console.log(error) }
}

exports.getDetails = async (req, res) => {
    const id = req.params.id;
    const condition = { id: id };

    try {
        const category = await Category.findOne({ where: condition })
        if (category) {
            return res.status(200).json({ category });
        } else { return res.status(404).json({ message: _response.empty }); }

    } catch (error) { console.log(error) }
}

exports.statusChange = async (req, res) => {
    const { status } = req.body;
    const id = req.params.id;
    const condition = { id: id };

    try {
        /* NO CONTENT */
        if (Object.keys(req.body).length === 0) { return res.status(400).json({ message: _response.no_content }) }

        /* UPDATE */
        const category = await Category.update({ status }, { where: condition })
        if (!category) { return res.status(200).json({ message: _response.failed }) };
        return res.status(200).json({ message: _response.update });
        
    } catch (error) { console.log(error) }
}
